(function(){
  ...
  function initFaqEvents(faqs) {
  	// click on a faq category
		faqs.faqsCategoriesContainer.addEventListener('click', function(event){
			let category = event.target.closest('.cd-faq__category');
			if(!category) return;
			let mq = getMq(faqs),
			...
			if(mq == 'mobile') { // on mobile, open faq panel
				...
				let selectedSection = faqs.faqContainer.getElementsByClassName('cd-faq__group--selected');
		...
		faqs.faqContainer.addEventListener('click', function(event){
			...
			let trigger = event.target.closest('.cd-faq__trigger');
			if(!trigger) return;
			event.preventDefault();
			...
			let initHeight = bool ? content.offsetHeight: 0,
  ...
  function updateCategory() { // update selected category -> show green rectangle to the left of the category
  	let selected = false;
		for(let i = 0; i < this.sections.length; i++) {
			let top = this.sections[i].getBoundingClientRect().top,
  ...
  let faqTemplate = document.getElementsByClassName('js-cd-faq'),
  	faqArray = [];
  if(faqTemplate.length > 0) {
		for(let i = 0; i < faqTemplate.length; i++) {
			faqArray.push(new FAQ(faqTemplate[i])); 
		}
  };
})();